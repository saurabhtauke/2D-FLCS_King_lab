function ptu_read_in_loop (filename, pathname) % Read PicoQuant Unified TTTR Files
...
%     [filename, pathname]=uigetfile('*.ptu', 'T-Mode data:');
    ...
    % write Header
    if (isT2)
      fprintf(fpout, '  record#,  Type, Ch,        TimeTag,             (, TrueTime/ps, ps, ), ch \n');
    else
      fprintf(fpout, '  record#,  Type, Ch,        TimeTag,             (, TrueTime/ns, ns, ), DTime, ch\n');
    end
    global cnt_ph;
    global cnt_ov;
    global cnt_ma;
...
function GotPhoton(TimeTag, Channel, DTime)
  ...
  if(isT2)
      ...
      % Edited: formatting changed by PK
      fprintf(fpout,'\n%10i, CHN, %i, %18.0f, (,%0.1f, ns,), %i, ch,', RecNum, Channel, TimeTag, (TimeTag * MeasDesc_GlobalResolution * 1e9), DTime);
%       fprintf(fpout,'\n%10i CHN %i %18.0f ( %0.1f ns ) %i ch', RecNum, Channel, TimeTag, (TimeTag * MeasDesc_GlobalResolution * 1e9), DTime);
